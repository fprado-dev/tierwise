...
// Zod validation schema (same as CreateModelModal)
...
type TModelProps = z.infer<typeof modelSchema> & {
...
interface UpdateModelModalProps {
  ...
  isLoading: boolean;
  initialValues: Partial<TModelProps> | null;
}
...
export function UpdateModelModal({
  ...
  const [model, setModel] = useState<TModelProps>({
    model_type: initialValues?.model_type ?? 'text',
    model_name: initialValues?.model_name ?? '',
    is_public: initialValues?.is_public ?? false,
    input_cost_per_million: initialValues?.input_cost_per_million ?? 0,
    output_cost_per_million: initialValues?.output_cost_per_million ?? 0,
    cost_per_image: initialValues?.cost_per_image ?? 0,
    cost_per_second: initialValues?.cost_per_second ?? 0,
    price_per_sec: initialValues?.price_per_sec ?? 0,
    price_per_hour: initialValues?.price_per_hour ?? 0,
    gpu_count: initialValues?.gpu_count ?? 0,
    cpu_multiplier: initialValues?.cpu_multiplier ?? 0,
    gpu_ram: initialValues?.gpu_ram ?? 0,
    ram: initialValues?.ram ?? 0,
  });
  ...
  const prepareModelPayload = (model: TModelProps) => {
    const base = {
